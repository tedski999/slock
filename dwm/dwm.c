...
enum { SchemeNorm, SchemeSel, SchemeHid,
       SchemeStatNorm, SchemeStatDim, SchemeStatGood, SchemeStatWarn }; /* color schemes */
...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, iscentered, isfloating, isurgent, neverfocus;
	int oldstate, isfullscreen, isterminal, noswallow, issticky;
	pid_t pid;
	Client *next;
	Client *snext;
	Client *swallowing;
...
{
	unsigned int i, n, x, w, click, tabw, occ = 0;
	Arg arg = {0};
	Client *c;
	Monitor *m;
...
// TODO: integrate with pertag (compare to traditional view)
void
...
drawbar(Monitor *m)
{
	int x, w, n = 0, sw = 0, scm, tabw, tw, remainder;
	...
	if (m == selmon) {
		...
		sw = wstext;
		x = drw_text(drw, m->ww - wstext, 0, lrpad / 2, bh, 0, "", 0, underlineWeight); /* to keep left padding clean */
		for (;;) {
			...
			*ts = '\0';
			if (*tp)
				x = drw_text(drw, x, 0, TTEXTW(tp), bh, 0, tp, 0, underlineWeight);
			if (ctmp == '\0')
				break;
		...
		drw_setscheme(drw, scheme[SchemeNorm]);
		drw_text(drw, x, 0, m->ww - x, bh, 0, "", 0, underlineWeight); /* to keep right padding clean */
	}
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i, underlineWeight);
		x += w;
	}
	w = blw = TEXTW(m->ltsymbol);
	drw_setscheme(drw, scheme[SchemeNorm]);
	x = drw_text(drw, x, 0, w, bh, lrpad / 2, m->ltsymbol, 0, underlineWeight);
	...
	if ((w = m->ww - sw - x) > bh) {
		...
		if (n > 0) {
			...
			for (c = m->clients; c; c = c->next) {
				...
				drw_text(drw, x, 0, tw, bh, lrpad / 2, c->name, 0, underlineWeight);
				x += tw;
			}
		}
	}
